...
router.post('/', (req, res) => {
  ...
  // Set default value for MODIFIED_BY if not provided
  const modifiedByValue = MODIFIED_BY || "System";

  const query = `
    INSERT INTO IT_PROJECTS 
    (PROJECT_NAME, START_DATE, TARGET_END_DATE, ACTUAL_END_DATE, CREATED_BY, MODIFIED_BY) 
  const values = [PROJECT_NAME, START_DATE, TARGET_END_DATE, ACTUAL_END_DATE, CREATED_BY, modifiedByValue];
  `;

  const values = [PROJECT_NAME, START_DATE, TARGET_END_DATE, ACTUAL_END_DATE, CREATED_BY, MODIFIED_BY];
